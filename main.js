...
const modelPath = 'https://storage.googleapis.com/tfjs-models/tfjs/mobilenet_v1_1.0_224/model.json';
...
const buildReadablePredictions = async function buildReadablePredictions(
  logits,
  maxPredictions
) {
  ...
  for (let i = 0; i < maxPredictionsIndices.length; i++) {
    topClassesAndProbs.push({
      className: IMAGENET_CLASSES[maxPredictionsIndices[i]],
...
const predictFromImage = async (image)=>{
  ...
  const predictions = await buildReadablePredictions(logits, 3);
...
const predictFromBase64 = base64 =>{

  return new Promise(function(resolve,reject){
    
    var image = document.createElement('img');
    image.onload = function () {
      resolve(predictFromImage(image));
    }
    image.src = base64;
...
const predictFromClick = async (event)=>{
  ...
  const predictions = await predictFromBase64(hd64);
